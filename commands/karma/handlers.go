...
type KarmaCommand struct {
	interfaces.IKarmaService
}

func (karmaCommand *KarmaCommand) AddKarmaCommandHandler(s *discordgo.Session, i *discordgo.InteractionCreate) (*discordgo.MessageEmbed, error) {
	...
	for _, opt := range commandOptions {
		if opt.Name == "user-option" {
			...
			karma, err := karmaCommand.AddKarma(opt.UserValue(s).ID, i.GuildID, 1)
			if err != nil {
...
func (karmaCommand *KarmaCommand) ShowKarmaCommandHandler(s *discordgo.Session, i *discordgo.InteractionCreate) (*discordgo.MessageEmbed, error) {
	options := i.ApplicationCommandData().Options
	...
	karma, err := karmaCommand.GetKarma(user.ID, i.GuildID)
	if err != nil {
		return embeds.CreateErrorEmbed(s, i, err), err
	}
...
func (karmaCommand *KarmaCommand) HelpKarmaCommandHandler(s *discordgo.Session, i *discordgo.InteractionCreate) (*discordgo.MessageEmbed, error) {
